...
@register.simple_tag
def get_nav_children(page, max_depth=1):
    """
    取得導航選單的子頁面
    """
    if not page:
    ...
    for child in children:
        ...
        # 如果需要更深層的子頁面
        if max_depth > 1:
            grandchildren = child.get_children().live().in_menu()
        ...
        nav_children.append(child_data)
    
    return nav_children


@register.filter
def has_nav_children(page):
    """
    檢查頁面是否有子頁面可以在導航中顯示
    """
    if not page:
...
def is_active_page(context, page, check_descendants=True):
    """
    檢查當前頁面是否為活躍狀態（用於導航選單高亮）
    """
    if not page:
        return False
    ...
    current_page = getattr(request, 'page', None)
    if not current_page:
        return False
    ...
    if check_descendants:
        return current_page.is_descendant_of(page)
